"""
app_basic_model 전체 테스트 자동 실행 스크립트
...
# ============================================
# 테스트 함수들 (JSON Body 방식)
# ============================================
def test_customers():
    print_header('Customers API (JSON Body)')
    ...
    # 수정 (JSON Body - id 포함)
    if cid:
        r = api_post_json('/update_customer', {'id': cid, 'cEmail': f'updated{uid}@test.com', 'cPhoneNumber': f'010-{uid}-2222', 'cName': '수정됨', 'cPassword': 'newpass'})
        s = print_test('수정 (JSON)', r.get('result') == 'OK')
        results['passed' if s else 'failed'] += 1
...
def test_employees():
    print_header('Employees API (이미지 포함 필수)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    uid = random.randint(10000, 99999)
...
def test_manufacturers():
    print_header('Manufacturers API (JSON Body)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    uid = random.randint(10000, 99999)
    ...
    r = api_post_json('/insert_manufacturer', {'mName': f'TestBrand{uid}'})
    mid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {mid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def test_product_bases():
    print_header('ProductBases API (JSON Body)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    ...
    uid = random.randint(10000, 99999)
    r = api_post_json('/insert_product_base', {'pName': f'테스트신발{uid}', 'pDescription': '설명', 'pColor': 'Red', 'pGender': 'Unisex', 'pStatus': 'active', 'pCategory': 'Running', 'pModelNumber': f'TEST-{uid}'})
    pbid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {pbid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def test_product_images():
    print_header('ProductImages API (JSON Body - 이미지 경로)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    ...
    # ProductImages는 이미지 경로(문자열)이므로 JSON 방식
    r = api_post_json('/insert_product_image', {'pbid': 1, 'imagePath': 'test/image.png'})
    imgid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {imgid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def test_products():
    print_header('Products API (JSON Body)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    ...
    uid = random.randint(10000, 99999)
    r = api_post_json('/insert_product', {'pbid': 1, 'mfid': 1, 'size': 265 + (uid % 10), 'basePrice': 99000, 'pQuantity': 10})
    pid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {pid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def test_purchases():
    print_header('Purchases API (JSON Body)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    uid = random.randint(10000, 99999)
    ...
    r = api_post_json('/insert_purchase', {'cid': 1, 'pickupDate': '2025-12-30 14:00', 'orderCode': f'TEST-{uid}', 'timeStamp': '2025-12-27 12:00'})
    pcid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {pcid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def test_purchase_items():
    print_header('PurchaseItems API (JSON Body)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    ...
    r = api_post_json('/insert_purchase_item', {'pid': 1, 'pcid': 1, 'pcQuantity': 2, 'pcStatus': '준비중'})
    piid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {piid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def test_login_histories():
    print_header('LoginHistories API (JSON Body)')
    results = {'passed': 0, 'failed': 0, 'tests': []}
    ...
    r = api_post_json('/insert_login_history', {'cid': 1, 'loginTime': '2025-12-27 12:00', 'lStatus': 'active', 'lVersion': 1.0, 'lAddress': '주소', 'lPaymentMethod': 'Card'})
    lid = r.get('id')
    s = print_test('추가 (JSON)', r.get('result') == 'OK', f"ID: {lid}")
    results['passed' if s else 'failed'] += 1
    results['tests'].append({'name': '추가', 'success': s})
...
def main():
    print('\n' + '🚀' * 20)
    print('\n   app_basic_model 전체 테스트 (JSON Body 방식)')
    print('\n' + '🚀' * 20)
