# app_basic_model API 테스트 결과 문서

> 테스트 일자: 2025-12-27
> 테스트 환경: FastAPI + MySQL (외부 서버)
> 서버 주소: http://127.0.0.1:8000
> 테스트 대상: app_basic_model 폴더 (**Model/JSON Body 방식** 학습용 단순화 API)
...
| 구분 | 설명 |
|------|------|
| **데이터 입력** | JSON Body (Pydantic Model 사용) |
| **이미지 경로** | `product_images.py` - JSON Body (문자열 경로) |
...
2. **UPDATE**: 두 가지 방식 제공
   - `update_customer` - 이미지 제외 (JSON Body)
...
### Manufacturers (제조사) - manufacturers.py [JSON Body]

| # | 엔드포인트 | 설명 | 결과 |
|---|-----------|------|------|
| 1 | GET `/select_manufacturers` | 전체 제조사 조회 | ✅ 성공 (2건) |
| 2 | GET `/select_manufacturer/{id}` | ID로 제조사 조회 | ✅ 성공 |
| 3 | POST `/insert_manufacturer` | 제조사 추가 (JSON) | ✅ 성공 |
| 4 | POST `/update_manufacturer` | 제조사 수정 (JSON) | ✅ 성공 |
| 5 | DELETE `/delete_manufacturer/{id}` | 제조사 삭제 | ✅ 성공 |

### ProductBases (제품 기본 정보) - product_bases.py [JSON Body]

| # | 엔드포인트 | 설명 | 결과 |
|---|-----------|------|------|
| 1 | GET `/select_product_bases` | 전체 ProductBase 조회 | ✅ 성공 (13건) |
| 2 | GET `/select_product_base/{id}` | ID로 ProductBase 조회 | ✅ 성공 |
| 3 | POST `/insert_product_base` | ProductBase 추가 (JSON) | ✅ 성공 |
| 4 | POST `/update_product_base` | ProductBase 수정 (JSON) | ✅ 성공 |
| 5 | DELETE `/delete_product_base/{id}` | ProductBase 삭제 | ✅ 성공 |

### ProductImages (제품 이미지) - product_images.py [JSON Body - 경로 문자열]
...
| 4 | POST `/insert_product_image` | 이미지 경로 추가 (JSON) | ✅ 성공 |
| 5 | POST `/update_product_image` | 이미지 경로 수정 (JSON) | ✅ 성공 |
| 6 | DELETE `/delete_product_image/{id}` | 이미지 삭제 | ✅ 성공 |

### Products (제품) - products.py [JSON Body]
...
| 4 | POST `/insert_product` | 제품 추가 (JSON) | ✅ 성공 |
| 5 | POST `/update_product` | 제품 수정 (JSON) | ✅ 성공 |
| 6 | DELETE `/delete_product/{id}` | 제품 삭제 | ✅ 성공 |

### Purchases (주문) - purchases.py [JSON Body]
...
| 4 | POST `/insert_purchase` | 주문 추가 (JSON) | ✅ 성공 |
| 5 | POST `/update_purchase` | 주문 수정 (JSON) | ✅ 성공 |
| 6 | DELETE `/delete_purchase/{id}` | 주문 삭제 | ✅ 성공 |

### PurchaseItems (주문 항목) - purchase_items.py [JSON Body]
...
| 4 | POST `/insert_purchase_item` | 항목 추가 (JSON) | ✅ 성공 |
| 5 | POST `/update_purchase_item` | 항목 수정 (JSON) | ✅ 성공 |
| 6 | POST `/update_purchase_item_status/{id}` | 상태만 수정 (JSON) | ✅ 성공 |
| 7 | DELETE `/delete_purchase_item/{id}` | 항목 삭제 | ✅ 성공 |

### LoginHistories (로그인 이력) - login_histories.py [JSON Body]
...
| 4 | POST `/insert_login_history` | 이력 추가 (JSON) | ✅ 성공 |
| 5 | POST `/update_login_history` | 이력 수정 (JSON) | ✅ 성공 |
| 6 | POST `/update_status_by_cid/{cid}` | 상태만 수정 (JSON) | ✅ 성공 |
| 7 | POST `/update_login_time_by_cid/{cid}` | 시간만 수정 (JSON) | ✅ 성공 |
...
python app_basic_model/TEST/run_all_tests.py
```

### 개별 파일 테스트

```bash
# 1. app_basic_model 폴더로 이동
cd /Users/cheng80/Git_Work/shoes_store_app/backend/app_basic_model

# 2. 테스트할 파일 실행 (터미널 1)
python customers.py

# 3. 테스트 실행 (터미널 2)
python TEST/test_app_basic_model.py customers
```
...
## 📁 app_basic_model 폴더 구조

```
app_basic_model/
├── TEST/
│   ├── run_all_tests.py           # 전체 자동 테스트
│   ├── test_app_basic_model.py    # 개별 테스트
...
├── manufacturers.py               # JSON Body
├── product_bases.py               # JSON Body
├── product_images.py             # JSON Body (경로 문자열)
├── products.py                    # JSON Body
├── purchases.py                   # JSON Body
├── purchase_items.py              # JSON Body
├── login_histories.py             # JSON Body
│
