"""
app_basic_model API 테스트 스크립트

각 파일을 개별 실행 후 이 스크립트로 테스트합니다.

방식: JSON Body (Model 방식)
      - 프로필 이미지(BLOB) 업로드만 Form/UploadFile 방식 유지

사용법:
    1. 테스트할 파일 실행: python customers.py
    2. 테스트 실행: python TEST/test_app_basic_model.py customers
...
def api_post_json(endpoint: str, data: dict) -> dict:
    """JSON Body 방식 POST"""
    try:
...
# ============================================
# Customers 테스트 (JSON Body)
# ============================================
def test_customers():
    print_header('Customers API 테스트 (JSON Body)')
    ...
    # 2. 고객 추가 (이미지 포함 필수 - Form + UploadFile)
    uid = random.randint(10000, 99999)
    new_customer = {
        'cEmail': f'test_model{uid}@test.com',
    ...
    if customer_id:
        ...
        success = result.get('result', {}).get('cEmail') == f'test_model{uid}@test.com'
    ...
    if customer_id:
        ...
        result = api_post_json('/update_customer', update_data)
        success = result.get('result') == 'OK'
        print_test('고객 수정 (이미지 제외)', success)
    
    # 4-2. 고객 수정 (이미지 포함 - Form + UploadFile)
    if customer_id:
        update_data = {
...
# ============================================
# Employees 테스트 (JSON Body)
# ============================================
def test_employees():
    print_header('Employees API 테스트 (JSON Body)')
    ...
    # 2. 직원 추가 (이미지 포함 필수 - Form + UploadFile)
    uid = random.randint(10000, 99999)
    new_emp = {
        'eEmail': f'emp_model{uid}@store.com',
    ...
    if emp_id:
        ...
        success = result.get('result', {}).get('eEmail') == f'emp_model{uid}@store.com'
    ...
    if emp_id:
        ...
        result = api_post_json('/update_employee', update_data)
        success = result.get('result') == 'OK'
        print_test('직원 수정 (이미지 제외)', success)
    
    # 4-2. 직원 수정 (이미지 포함 - Form + UploadFile)
    if emp_id:
        update_data = {
            'employee_id': emp_id,
...
# ============================================
# Manufacturers 테스트 (JSON Body)
# ============================================
def test_manufacturers():
    print_header('Manufacturers API 테스트 (JSON Body)')
    ...
    # 2. 제조사 추가 (JSON)
    result = api_post_json('/insert_manufacturer', {'mName': 'TestBrandModel'})
    mf_id = result.get('id')
    ...
    if mf_id:
        ...
        success = result.get('result', {}).get('mName') == 'TestBrandModel'
        print_test('ID로 제조사 조회', success)
    
    # 4. 제조사 수정 (JSON)
    if mf_id:
        result = api_post_json('/update_manufacturer', {'id': mf_id, 'mName': 'UpdatedBrand'})
        success = result.get('result') == 'OK'
...
# ============================================
# ProductBases 테스트 (JSON Body)
# ============================================
def test_product_bases():
    print_header('ProductBases API 테스트 (JSON Body)')
    ...
    pb_data = {
        ...
        'pStatus': '',
        'pCategory': 'Running',
        'pModelNumber': 'TEST-MODEL-001'
    }
    result = api_post_json('/insert_product_base', pb_data)
    pb_id = result.get('id')
    ...
    if pb_id:
        ...
        success = result.get('result', {}).get('pName') == '테스트신발Model'
...
# ============================================
# ProductImages 테스트 (JSON Body - 이미지 경로 문자열)
# ============================================
def test_product_images():
    print_header('ProductImages API 테스트 (JSON Body - 이미지 경로)')
    ...
    # 2. ProductImage 추가 (JSON - 이미지 경로는 문자열)
    result = api_post_json('/insert_product_image', {'pbid': 1, 'imagePath': 'test/image_model.png'})
    img_id = result.get('id')
    success = result.get('result') == 'OK' and img_id is not None
    print_test('ProductImage 추가 (JSON)', success, f"ID: {img_id}")
...
# ============================================
# Products 테스트 (JSON Body)
# ============================================
def test_products():
    print_header('Products API 테스트 (JSON Body)')
    ...
    result = api_post_json('/insert_product', prod_data)
    ...
    if prod_id:
        ...
        success = result.get('result', {}).get('size') == 275
...
# ============================================
# Purchases 테스트 (JSON Body)
# ============================================
def test_purchases():
    print_header('Purchases API 테스트 (JSON Body)')
    ...
    purchase_data = {
        ...
        'orderCode': 'TEST-MODEL-ORDER-001',
        'timeStamp': '2025-12-27 12:00'
    }
    result = api_post_json('/insert_purchase', purchase_data)
    ...
    if purchase_id:
        ...
        success = result.get('result', {}).get('orderCode') == 'TEST-MODEL-ORDER-001'
...
# ============================================
# PurchaseItems 테스트 (JSON Body)
# ============================================
def test_purchase_items():
    print_header('PurchaseItems API 테스트 (JSON Body)')
    ...
    result = api_post_json('/insert_purchase_item', item_data)
    ...
    # 4. 상태만 수정 (JSON)
    if item_id:
        result = api_post_json(f'/update_purchase_item_status/{item_id}', {'pcStatus': '배송 중'})
        success = result.get('result') == 'OK'
        print_test('PurchaseItem 상태 수정', success)
...
# ============================================
# LoginHistories 테스트 (JSON Body)
# ============================================
def test_login_histories():
    print_header('LoginHistories API 테스트 (JSON Body)')
    ...
    result = api_post_json('/insert_login_history', login_data)
    ...
    # 4. 상태만 수정 (JSON)
    result = api_post_json('/update_status_by_cid/1', {'lStatus': 'inactive'})
    success = result.get('result') == 'OK'
    print_test('cid별 상태 수정', success)
...
def main():
    if len(sys.argv) < 2:
        print('사용법: python TEST/test_app_basic_model.py <module_name>')
        print('\n사용 가능한 모듈:')
        for name in TEST_MODULES:
            print(f'  - {name}')
        print('\n예시: python TEST/test_app_basic_model.py customers')
        return
    ...
    print(f'\n🚀 {module_name} 테스트 시작... (Model/JSON Body 방식)')
    print(f'   서버: {BASE_URL}')
