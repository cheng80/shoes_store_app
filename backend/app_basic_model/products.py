...
async def insert_product(product: ProductCreate):
    try:
        ...
        sql = """
            ...
            VALUES (%s, %s, %s, %s, %s)
        """
        curs.execute(sql, (product.pbid, product.mfid, product.size, product.basePrice, product.pQuantity))
        conn.commit()
...
async def update_product(product: ProductUpdate):
    try:
        ...
        sql = """
            ...
            WHERE id=%s
        """
        curs.execute(sql, (product.pbid, product.mfid, product.size, product.basePrice, product.pQuantity, product.id))
        conn.commit()
        conn.close()
        return {"result": "OK"}
    except Exception as e:
