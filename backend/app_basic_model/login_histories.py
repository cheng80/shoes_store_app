"""
LoginHistory API - 로그인 이력 CRUD (Model 방식)
...
class LoginTimeUpdate(BaseModel):
    loginTime: str
...
async def insert_login_history(history: LoginHistoryCreate):
    try:
        conn = connect_db()
        curs = conn.cursor()
        sql = """
        ...
        """
        curs.execute(sql, (
            history.cid, history.loginTime, history.lStatus,
            history.lVersion, history.lAddress, history.lPaymentMethod
        ))
        conn.commit()
...
async def update_login_history(history: LoginHistoryUpdate):
    try:
        conn = connect_db()
        curs = conn.cursor()
        sql = """
        ...
        curs.execute(sql, (
            history.cid, history.loginTime, history.lStatus,
            history.lVersion, history.lAddress, history.lPaymentMethod, history.id
        ))
        conn.commit()
        conn.close()
        return {"result": "OK"}
...
# ============================================
# 고객 ID로 상태만 수정 (JSON Body)
# ============================================
@app.post("/update_status_by_cid/{cid}")
async def update_status_by_cid(cid: int, status: StatusUpdate):
    try:
        conn = connect_db()
        curs = conn.cursor()
        sql = "UPDATE LoginHistory SET lStatus=%s WHERE cid=%s"
        curs.execute(sql, (status.lStatus, cid))
        conn.commit()
...
# ============================================
# 고객 ID로 로그인 시간만 수정 (JSON Body)
# ============================================
@app.post("/update_login_time_by_cid/{cid}")
async def update_login_time_by_cid(cid: int, time_update: LoginTimeUpdate):
    try:
        conn = connect_db()
        curs = conn.cursor()
        sql = "UPDATE LoginHistory SET loginTime=%s WHERE cid=%s"
        curs.execute(sql, (time_update.loginTime, cid))
        conn.commit()
        affected_rows = curs.rowcount
        conn.close()
        return {"result": "OK", "affected_rows": affected_rows}
